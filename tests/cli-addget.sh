...
test-step "Fails to retrieve secret with new private key"
...
ls_secret_add hello.world "$SECRET" "$PRIVKEY"
test-expect "$(ls_secret_get hello.world "$PRIVKEY")" "$SECRET"

test-info "Fails to retrieve secret with default private key"
if ls_secret_get hello.world; then
	test-fail "Should not decode secret"
else
	test-ok
fi
