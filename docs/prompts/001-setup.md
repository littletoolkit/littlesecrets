...
- secrets are shared in `.littlesecrets` users register their ssh public key
- (any format accepted) in `.littlesecrets/users/${USER}.pubkey` (the key would
- likely be normalised) a secret is encrypted in
- `.littlesecrets/secrets/${SECRET}`, where `secret.enc` is the encrypted secret
...
- `.littlesecrets/users/$OTHERUSER.pubkey`, saving the result in the secret's
