...
We're going to implement the asymmetric crypto backend in `src/go/crypto.go`:

- Start by implementing the AsmCrypto interface,
- Implement AsmCryptoRSA, ensuring a  minimum 3072 bits for future-proofing
...
in `tests/crypto-asm-rsa.go` with the following tests:
...
- Formats: validates the loading from supported key formats.
    ...
    - For each keypair, load both keys and apply the Encryption test with these keys
